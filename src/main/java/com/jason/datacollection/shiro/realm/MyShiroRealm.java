package com.jason.datacollection.shiro.realm;
...
public class MyShiroRealm extends AuthorizingRealm {
    ...
    @Resource(type = KUserService.class)
    ...
    protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principalCollection) {
        ...
        KUser KUser = (KUser) principalCollection.getPrimaryPrincipal();
        // 把角色和权限添加到授权器
        SimpleAuthorizationInfo authorizationInfo = new SimpleAuthorizationInfo();
        if ("admin".equals(KUser.getAccount())) {
            authorizationInfo.addRole("admin");
        } else {
            authorizationInfo.addRole("KUser");
        }
    ...
    protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authenticationToken) throws AuthenticationException {
        ...
        // 通过用户名去数据库中获取密码凭证
        KUser KUser = userService.getUserByAccount(userName);
        if (KUser == null) {
            throw new UnknownAccountException();
        }
        // 把查询的信息放入验证对象中, getName()是获取当前Realm对象的名称
        SimpleAuthenticationInfo info = new SimpleAuthenticationInfo(KUser, KUser.getPassword(), getName());
