...
function getJobCountInfo() {
    $.ajax({
        type: 'GET',
        async: false,
        url: '/dataCollection/job/monitor/countJob.do',
        data: {},
        success: function (data) {
            if (data.success) {
                var count = data.result;
                $("#allJob").text(count.total);
...
function getMonitorStatus() {
    $.ajax({
        type: 'GET',
        async: false,
        url: '/dataCollection/enum/runStatus.do',
        data: {},
        success: function (data) {
            var list = data.result;
            for (var i=0; i<list.length; i++){
...
function getTransMonitorList() {
    $('#jobMonitorList').bootstrapTable({
        url: '/dataCollection/job/monitor/findJobMonitorListByPage.do',            //请求后台的URL（*）
        method: 'POST',            //请求方式（*）
...
function actionFormatter(value, row, index) {
    return ['<a class="btn  btn-xs" id="viewDetail" type="button" data-id="'+ row.monitorJobId +'">&nbsp;查看详细</a>'].join('');
}
...
function bindButton() {
    ...
    $('#jobMonitorList').delegate('#viewDetail','click',function(e) {
        ...
        location.href = "../record/j-list.html?jobId=" + jobId;
    });
}
