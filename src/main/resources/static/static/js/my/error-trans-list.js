...
function getTransErrorList() {
    $('#transerrorList').bootstrapTable({
        url: '/dataCollection/trans/monitor/findErrorRecordList.do',            //请求后台的URL（*）
        method: 'POST',            //请求方式（*）
...
function search() {
    $('#transerrorList').bootstrapTable('refresh');
}
...
function transNameFormatter(value, row, index) {
    ...
    function bindButton() {
        // 查看日志
        $('#transerrorList').delegate('#view','click',function(e) {
            var $target = $(e.currentTarget);
            var transRecordId = $target.data('id');
            $.ajax({
                type: 'GET',
                async: false,
                url: '/dataCollection/trans/monitor/viewTransRecordDetail.do?transRecordId=' + transRecordId,
                data: {},
                success: function (data) {
                    if (data.success) {
                        layer.open({
        ...
        // 日志下载
        $('#transerrorList').delegate('#download','click',function(e) {
            var $target = $(e.currentTarget);
            var transRecordId = $target.data('id');
            layer.confirm('确定下载该日志记录？', {
                ...
                function (index) {
                    ...
                    form.attr('action', '/dataCollection/trans/monitor/downloadTransRecord.do');
                    var $recordId = $('<input>');
