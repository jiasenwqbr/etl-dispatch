    ...
    $("#RepositoryJobForm").validate({
        ...
        submitHandler: function (form) {
            ...
            $.ajax({
                type: 'PUT',
                async: false,
                url: '/dataCollection/job/update.do',
...
function getTransLogLevel() {
    $.ajax({
        type: 'GET',
        async: false,
        url: '/dataCollection/enum/logLevel.do',
        data: {},
...
function getQuartz() {
    $.ajax({
        type: 'GET',
        async: false,
        url: '/dataCollection/quartz/findQuartzList.do',
        data: {},
        success: function (data) {
            var list = data.result;
            for (var i = 0; i < list.length; i++) {
...
function initData() {
    ...
    if (jobId && jobId !== "") {
        $.ajax({
            type: 'GET',
            async: false,
            url: '/dataCollection/job/getJobDetail.do?id=' + jobId,
            data: {},
            success: function (data) {
                if (data.success) {
                    var job = data.result;
