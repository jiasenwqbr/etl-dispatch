...
function getTransCountInfo() {
    $.ajax({
        type: 'GET',
        async: false,
        url: '/dataCollection/script/monitor/countTrans',
        data: {},
...
function getMonitorStatus() {
    $.ajax({
        type: 'GET',
        async: false,
        url: '/dataCollection/enum/runStatus',
        data: {},
        success: function (data) {
            var list = data.result;
            for (var i=0; i<list.length; i++){
...
function actionFormatter(value, row, index) {
    return ['<a class="btn  btn-xs" id="viewDetail" type="button" data-id="'+ row.monitorScriptId +'">&nbsp;查看详细</a>'].join('');
}
...
function bindButton() {
    ...
    $('#transMonitorList').delegate('#viewDetail','click',function(e) {
        ...
        location.href = "../record/t-list.html?transId=" + transId;
    });
}
