$(document).ready(function () {
    $("#id").val(getQueryVariable("repositoryId"));
    // 初始化隐藏动态表单
    hideForm();
    // 资源库下拉列表
    getRepType();
...
function initData() {
    ...
    if (repositoryId && repositoryId !== "") {
        $.ajax({
            type: 'GET',
            async: true,
            url: '/dataCollection/repository/getRepositoryDetail?id=' + repositoryId,
            data: {},
            success: function (data) {
                if (data.success) {
        ...
        submitHandler: function (form) {
            ...
            if (testConnection()) {
              ...
              /*  console.log(data);
                ...
                var repository = {};
                repository.id = 'KettleDatabaseRepository';
                repository.name = data.repName;
                repository.description = data.repName;
                repository.is_default = 'false';
                repository.connection = connecetionName;
                ...
                $.ajax({
                    type: 'POST',
                    async: false,
                    url: kettleUtilUrl + '/repository/edit',
                    ...
                    success: function (data) {
                        ...
                        if (data.code == '0000') {*/
                            ...
                            $.ajax({
                                type: 'POST',
                                async: false,
                                url: '/dataCollection/repository/update',
                                data: JSON.stringify(data),
                                contentType: "application/json;charset=UTF-8",
